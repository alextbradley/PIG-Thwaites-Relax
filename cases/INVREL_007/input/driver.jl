...
read!("Inverse_2km_h_mask_clip.bin",h_mask)
h_mask.=ntoh.(h_mask)

u_iszero=Array{Float64}(undef,nx+1,ny);
read!("Inverse_2km_uiszero_clip.bin",u_iszero)
u_iszero.=ntoh.(u_iszero)

v_iszero=Array{Float64}(undef,nx,ny+1);
read!("Inverse_2km_viszero_clip.bin",v_iszero)
v_iszero.=ntoh.(v_iszero)

sigma_grid=Array{Float64}(undef,nσ);
read!("Inverse_2km_sigma_grid.bin",sigma_grid)
...
read!("Inverse_2km_bed_clip_noNan.bin",bed)
bed.=ntoh.(bed)

h=Array{Float64}(undef,nx,ny);
read!("thickness_interp_INVREL001_3km_at_700yrs_to2km.bin",h)
h.=ntoh.(h)

viscosity=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_viscosity3D_clip_noNan.bin",viscosity)
viscosity.=ntoh.(viscosity)

temp=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_3Dtemp_clip_noNan.bin",temp)
temp.=ntoh.(temp)

damage=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_damage3D_clip_noNan.bin",damage)
damage.=ntoh.(damage)
 
weertman_c=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_WeertmanC_clip_adjusted_noNan.bin",weertman_c)
weertman_c.=ntoh.(weertman_c)

accumulation_rate=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_accumulation_clip_noNan.bin",accumulation_rate)
accumulation_rate.=ntoh.(accumulation_rate)

dhdt=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_dhdt_clip_noNan.bin",dhdt)
dhdt.=ntoh.(dhdt)

gu_u=Array{Float64}(undef,nx+1,ny);
read!("uvel_interp_INVREL001_3km_at_700yrs_to2km.bin",gu_u)
gu_u.=ntoh.(gu_u)

gv_v=Array{Float64}(undef,nx,ny+1);
read!("vvel_interp_INVREL001_3km_at_700yrs_to2km.bin",gv_v)
gv_v.=ntoh.(gv_v)
