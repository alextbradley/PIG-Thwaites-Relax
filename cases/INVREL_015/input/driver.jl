...
read!("Inverse_2km_h_mask_clip_BedmachineV2.bin",h_mask)
h_mask.=ntoh.(h_mask)

u_iszero=Array{Float64}(undef,nx+1,ny);
read!("Inverse_2km_uiszero_clip_BedmachineV2.bin",u_iszero)
u_iszero.=ntoh.(u_iszero)

v_iszero=Array{Float64}(undef,nx,ny+1);
read!("Inverse_2km_viszero_clip_BedmachineV2.bin",v_iszero)
...
read!("Inverse_2km_bed_clip_noNan_BedmachineV2.bin",bed)
bed.=ntoh.(bed)

h=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_thickness_clip_noNan_BedmachineV2.bin",h)
h.=ntoh.(h)

viscosity=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_viscosity3D_clip_noNan_BedmachineV2.bin",viscosity)
viscosity.=ntoh.(viscosity)

temp=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_3Dtemp_clip_noNan_BedmachineV2.bin",temp)
temp.=ntoh.(temp)

damage=Array{Float64}(undef,nx,ny,nσ);
read!("Inverse_2km_damage3D_clip_noNan_BedmachineV2.bin",damage)
damage.=ntoh.(damage)
 
weertman_c=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_WeertmanC_clip_adjusted_noNan_BedmachineV2.bin",weertman_c)
weertman_c.=ntoh.(weertman_c)

accumulation_rate=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_accumulation_clip_noNan_BedmachineV2.bin",accumulation_rate)
accumulation_rate.=ntoh.(accumulation_rate)

dhdt=Array{Float64}(undef,nx,ny);
read!("Inverse_2km_dhdt_clip_noNan_BedmachineV2.bin",dhdt)
dhdt.=ntoh.(dhdt)

gu_u=Array{Float64}(undef,nx+1,ny);
read!("Inverse_2km_u_velocs_clip_noNan_BedmachineV2.bin",gu_u)
gu_u.=ntoh.(gu_u)

gv_v=Array{Float64}(undef,nx,ny+1);
read!("Inverse_2km_v_velocs_clip_noNan_BedmachineV2.bin",gv_v)
gv_v.=ntoh.(gv_v)
